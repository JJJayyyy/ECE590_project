...
object_list = ['rect', 'circle', 'triangle']
...
class PlaceDrawer(object):
    ...
    def forward(pos,
        ...
        try:
            ...
            def draw_tri(x1, y1, x2, y2, flip=False):
                if flip:
                    ctx.move_to(x1, y1)  
                    ctx.line_to(x2, y1) 
                    ctx.line_to((x1 + x2) / 2, y2)  # Apex
                else:
                    ctx.move_to(x1, y2)  
                    ctx.line_to(x2, y2)
            ...
            def draw_circle(x1, y1, x2, y2):
                center_x = (x1 + x2) / 2
                center_y = (y1 + y2) / 2
                radius = math.sqrt((x2 - x1) ** 2 + (y2 - y1) ** 2) / (2 * math.sqrt(2))
            ...
            for i in range(num_movable_nodes):
                if i < num_qubit:
                    if i % 3 == 0:
                        ...
                        draw_circle(node_xl[i], node_yl[i], node_xh[i], node_yh[i])
                    else:
                        if iter == 0:
                            flip = False
                        else:
                        ...
                        draw_tri(node_xl[i], node_yl[i], node_xh[i], node_yh[i], flip)
